            ...
            <div className="grid gap-6 mb-14">
              {roles.map((role) => (
                <button
                  ...
                  className={`
                    group relative flex items-center justify-between p-8 rounded-[2.5rem] border transition-all duration-500
                ...
                >
                  <div className="flex items-center gap-8">
                    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl font-black transition-all ${selectedRole === role.id ? `bg-${role.color}-500 text-black` : 'bg-white/5 text-slate-700'}`}>
                      {role.icon}
                    </div>
                    <div className="text-left">
                      <p className={`text-sm font-black uppercase tracking-widest ${selectedRole === role.id ? `text-${role.color}-400` : 'text-slate-500'}`}>
                        {role.label}
                      </p>
                      <p className="text-[11px] text-slate-600 mt-2 font-bold italic leading-tight">{role.desc}</p>
                    </div>
                  </div>
                </button>
              ))}
