  ...
  const handleSendMessage = async (e?: React.FormEvent, customText?: string) => {
    ...
    try {
      ...
      if (!apiKey) {
        ...
        console.warn("⚠️ [NEURAL_CORE] No API Key detected. Engaging simulation protocols.");
        await new Promise(resolve => setTimeout(resolve, 1500 + Math.random() * 1000));

        const mockResponses = [
          `Analysis of Case ${activeCase.loanId} indicates a 94.2% probability of settlement via deferred liquidity injection. I recommend initiating Protocol Omega-7.`,
          "I have cross-referenced the liquidity pools. Changing the negotiation strategy to 'Soft_Push' increases recovery odds by 12%. Shall I execute?",
          "The applicant's biometric telemetry suggests high stress. Acknowledging their constraints while holding firm on the principal amount is the optimal tactical approach."
        ];

        const textResponse = mockResponses[Math.floor(Math.random() * mockResponses.length)];
      ...
      // Even on error, provide a fallback response to keep the UI alive
      setMessages(prev => [...prev, {
        id: `sys-${Date.now()}`,
        role: 'AI_AGENT',
        text: '⚠️ Neural Link Unstable. Switching to Heuristic Analysis: Recommendation -> HOLD POSITION.',
        timestamp: new Date().toLocaleTimeString()
      }]);
