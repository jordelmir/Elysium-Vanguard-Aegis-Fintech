...
const ApplicantFlow: React.FC<ApplicantFlowProps> = ({ onStepChange, riskData }) => {
  ...
  return (
    <div className="w-full h-full max-w-6xl mx-auto flex flex-col md:flex-row bg-[#02040a] md:bg-slate-950 md:rounded-[3rem] border-0 md:border md:border-white/5 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-700">
      
      {/* Sidebar: Progress & Context - Hidden on Mobile */}
      <div className="hidden lg:flex lg:w-72 bg-slate-900/50 p-8 border-r border-white/5 flex-col justify-between shrink-0">
        <div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-y-auto relative">
        ...
        {/* Mobile Progress Bar */}
        <div className="sticky top-0 p-4 md:p-8 z-40 flex flex-col gap-2 bg-[#02040a] md:bg-slate-950/80 backdrop-blur-xl border-b border-white/5">
           <div className="flex justify-between items-center">
             ...
             <div className="w-24 md:w-32 h-1.5 bg-slate-900 rounded-full overflow-hidden">
          ...
          {currentStep === APPLICANT_FLOW_STEP.IDENTITY_SCAN && (
            <div className="animate-in slide-in-from-bottom-8 duration-500 max-w-2xl mx-auto w-full text-center md:text-left">
              ...
              <button 
                ...
                disabled={isScanning}
                className={`mt-8 md:mt-12 w-full py-5 md:py-7 rounded-2xl md:rounded-[2rem] font-black uppercase tracking-[0.4em] transition-all transform active:scale-95 shadow-2xl ${isVerified ? 'bg-cyan-600 hover:bg-cyan-500 text-white shadow-cyan-900/40' : 'bg-white text-black hover:bg-slate-200'}`}
              >
            ...
            <div className="animate-in fade-in slide-in-from-right-12 duration-500 text-center max-w-lg mx-auto w-full">
               ...
               <div className="relative mx-auto w-48 sm:w-64 md:w-80 aspect-square">
                 <div className="absolute inset-0 rounded-full border-2 md:border-4 border-slate-900 overflow-hidden bg-slate-950 shadow-2xl">
                    ...
                    <div className="absolute inset-0 border-[10px] md:border-[20px] border-slate-950 rounded-full"></div>
                 </div>
               ...
               <button onClick={next} className="mt-8 md:mt-12 w-full py-5 md:py-7 bg-cyan-600 text-white font-black uppercase tracking-[0.4em] rounded-2xl md:rounded-[2rem] hover:bg-cyan-500 transition-all shadow-2xl shadow-cyan-900/40">Verify_Biological_Vector</button>
            </div>
          )}
          ...
          {currentStep === APPLICANT_FLOW_STEP.BANK_CONNECT && (
            <div className="animate-in fade-in slide-in-from-right-12 duration-500 max-w-2xl mx-auto w-full">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                 {[
                   ...
                   <button 
                    ...
                    className={`p-4 md:p-6 rounded-2xl md:rounded-3xl border transition-all flex items-center justify-between group ${selectedBank === bank.id ? 'border-cyan-500 bg-cyan-500/10 shadow-lg' : 'border-white/5 bg-slate-900/40'}`}
                   >
               ...
               <button 
                 ...
                 className={`mt-10 md:mt-14 w-full py-5 md:py-7 font-black uppercase tracking-[0.4em] rounded-2xl md:rounded-[2rem] transition-all shadow-2xl ${selectedBank ? 'bg-cyan-600 hover:bg-cyan-500 text-white shadow-cyan-900/40' : 'bg-slate-900 text-slate-700 cursor-not-allowed'}`}
               >
                 Authorize_Nexus
               </button>
            </div>
               ...
               <div className="flex flex-col items-center">
                  <div className="text-center mb-8 md:mb-16">
                     ...
                     <div className="flex items-baseline justify-center gap-1 md:gap-2">
                        <span className="text-6xl sm:text-7xl md:text-9xl font-black text-white tracking-tighter">${loanAmount}</span>
                  ...
                  <div className="w-full relative px-2 mb-8 md:mb-12">
                     <input 
                        ...
                        className="w-full h-3 bg-slate-900 rounded-full appearance-none cursor-pointer accent-cyan-500 mb-6 md:mb-8"
                     />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 w-full">
                     <div className="bg-slate-900/40 p-4 md:p-6 rounded-2xl md:rounded-[2rem] border border-white/5 flex flex-col items-center">
                     ...
                     <div className="bg-slate-900/40 p-4 md:p-6 rounded-2xl md:rounded-[2rem] border border-white/5 flex flex-col items-center">
                        ...
                        <span className="text-xl md:text-3xl font-black text-emerald-500 tracking-tighter">1.42%</span>
                     </div>
                  </div>
               </div>

               <button onClick={next} className="mt-10 md:mt-14 w-full py-5 md:py-7 bg-emerald-600 text-white font-black uppercase tracking-[0.4em] rounded-2xl md:rounded-[2rem] hover:bg-emerald-500 transition-all shadow-2xl active:scale-95">Commit_Capital</button>
            </div>
          )}
               ...
               <div className="bg-slate-900/60 p-6 md:p-10 rounded-2xl md:rounded-[3rem] border border-white/10 text-left mb-8 md:mb-12">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8">
                     ...
                     <div>
                        ...
                        <span className="text-sm md:text-xl font-black text-emerald-500 tracking-tight">PAdES_V2</span>
                     </div>
